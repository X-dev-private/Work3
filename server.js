...
// Endpoint para buscar todas as informações dos usuários
...
// Endpoint para listar objetos na pasta 'daos'
app.get('/daos', async (req, res) => {
  ...
  try {
    ...
    res.status(500).send('Failed to fetch daos');
  }
});

// Endpoint para obter um DAO específico
app.get('/dao/:key', async (req, res) => {
  const { key } = req.params;
  ...
  try {
    const data = await s3.getObject(params).promise();
    const daoData = JSON.parse(data.Body.toString('utf-8'));
    res.setHeader('Content-Type', 'application/json');
    res.json(daoData);
  } catch (error) {
